...
scores <- dat %>% 
  ...
  rowwise() %>% 
  mutate(instruction_score = sum(c_across(starts_with("b_")), na.rm = TRUE),
...
#test turn cols to list to drop strings
test_list <- list()
for(i in 1:ncol(full_set)) {
  test_list[[i]] <- full_set[ ,i]
}
names(test_list) <- colnames(full_set)
#map through
pmap_chr(test_list, ~ str_remove(.x, c("b_", "c_", "d_", "e_")))

map_chr(test_vec[[names]], ~ .x %>% 
      rename_all(~ str_remove(.x, c("b_", "c_", "d_", "e_"))))

save <- map_chr(test_vec[["names"]], ~ str_remove(.x, c("^b_", "^c_", "^d_", "^e_")))

map(full_set$)

test_vec <- attributes(test_list)
